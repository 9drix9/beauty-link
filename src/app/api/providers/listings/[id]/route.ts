...
export async function GET(
  ...
  try {
    ...
    logger.error("LISTING_FETCH_FAILED", { listingId: params.id, error: error instanceof Error ? error.message : "Unknown error" });
...
export async function PATCH(
  ...
  try {
    ...
    if (body.status) {
      logger.info("LISTING_STATUS_CHANGED", {
        listingId: params.id,
    ...
    logger.error("LISTING_UPDATE_FAILED", { listingId: params.id, error: error instanceof Error ? error.message : "Unknown error" });
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    logger.info("LISTING_DELETED", {
      listingId: listing.id,
      professionalId: profile.id,
      cancelledBookings: listing.bookings.length,
    });

    return NextResponse.json({ success: true });
  } catch (error) {
    logger.error("LISTING_DELETE_FAILED", { listingId: params.id, error: error instanceof Error ? error.message : "Unknown error" });
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
